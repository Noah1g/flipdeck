...
const I18N = {
  de:{
    ...
    "nav.logout":"Abmelden",
    "login.subtitle":"Hub fürs alternative Reselling","login.user":"Benutzername oder E-Mail","login.pass":"Passwort","login.remember":"Angemeldet bleiben","login.btn":"Anmelden","login.err":"Username oder Passwort falsch.",
  ...
  en:{
    ...
    "nav.logout":"Log out",
    "login.subtitle":"Hub for alternative reselling","login.user":"Username or email","login.pass":"Password","login.remember":"Stay signed in","login.btn":"Sign in","login.err":"Wrong username or password.",
...
function authErrorDE(msg){
  ...
  if(m.includes("already registered") || m.includes("already been registered")) return "Für diese E-Mail existiert bereits ein Konto. Wechsle oben auf „Anmelden“ oder setze dein Passwort zurück.";
    ...
    el.innerHTML=`
      ...
      <div class="inv-body" ${open?"":"hidden"}>
        <div class="inv-body-inner">
          ...
          <p class="c-sub text-[11.5px] mono mb-3">${lang==="en"?"Ship":"Versand"} ${eur(it.ship)} · ${lang==="en"?"Fees":"Gebühren"} ${combined.toLocaleString("de-DE")} % · ${lang==="en"?"Capital":"Kapital"} ${eur(it.ek*it.qty)}${it.orderDate?` · Geliefert ${new Date(it.orderDate).toLocaleDateString("de-DE")}`:""}</p>
          ${mpCompareHTML(it, st)}
