...
function dealScoreInfoHTML(ds){
  ...
  // Marktplatz-Aufschlüsselung: derselbe Artikel schneidet je nach Verkaufsort anders ab
  let comp="";
  if(ds && isFinite(ds.vk) && ds.vk>0 && isFinite(ds.ek)){
    const V=vatF(), vk=ds.vk, ek=ds.ek, ship=ds.ship||0, comb=isFinite(ds.comb)?ds.comb:0;
    const mk=profit=>dealGrade(profit, ek);
    const eComm = mk(vk - (transFee(vk)+vk*comb/100*V) - ek - ship);
    const ePriv = mk(vk - ek - ship);                 // eBay Privat, Inland = 0 Gebühr
    const kfld  = mk(vk - vk*13/100*V - ek - ship);   // Kaufland Standard 13 %
    const cline=(label,r)=>`<div style="display:flex;align-items:center;justify-content:space-between;gap:9px;margin:6px 0"><span style="color:var(--text)">${label}</span><span style="display:inline-flex;align-items:center;gap:8px"><span class="c-sub" style="font-size:12px">ROI ${pct(r.roi)}</span><span class="pill" style="border:1px solid color-mix(in srgb,${r.col} 45%,var(--line));color:${r.col};background:color-mix(in srgb,${r.col} 12%,transparent);font-weight:800;min-width:26px;text-align:center">${r.g}</span></span></div>`;
    comp = `<div style="border-top:1px solid var(--line);margin-top:14px;padding-top:12px">
      ...
      ${cline("eBay gewerblich", eComm)}
      ${cline("eBay Privat (Inland)", ePriv)}
      ${cline("Kaufland (Standard 13 %)", kfld)}
      <p class="c-sub text-[11px] mt-2 leading-relaxed">Der ◆-Score auf der Karte ist die Schätzung für <b>eBay gewerblich</b>. Beim Verkauf-Eintragen zählt der real gewählte Marktplatz.</p></div>`;
  }
...
function renderInventory(){ const list=$("#inv-list"); applyFeatCfg(); scheduleCalFeedSync();
  ...
  $$("#inv-list .deal-pill").forEach(b=>b.addEventListener("click",e=>{ e.stopPropagation();
    openInfoModal("Deal-Score", dealScoreInfoHTML({ g:b.dataset.grade, roi:parseFloat(b.dataset.roi), col:b.dataset.col, vk:parseFloat(b.dataset.vk), ek:parseFloat(b.dataset.ek), ship:parseFloat(b.dataset.ship), comb:parseFloat(b.dataset.comb) })); }));
